...
public class BedTntBlock extends CustomTntBlock{

    public static final Block ENTRY = new BedTntBlock(AbstractBlock.Properties.create(Material.TNT).zeroHardnessAndResistance().sound(SoundType.CROP)).setRegistryName("tds", "bed_tnt");

    public BedTntBlock(Properties properties) {
        super(properties);
    }
    ...
    public void catchFire(BlockState state, World world, BlockPos pos, @Nullable Direction face, @Nullable LivingEntity igniter) {
        if (!world.isRemote) {
            com.superruper1209.tds.Common.Entities.BedTnt tntentity = new com.superruper1209.tds.Common.Entities.BedTnt(world, (double)pos.getX() + 0.5D, (double)pos.getY(), (double)pos.getZ() + 0.5D, igniter);
    ...
    public void onExplosionDestroy(World worldIn, BlockPos pos, Explosion explosionIn) {
        if (!worldIn.isRemote && !worldIn.getDimensionType().doesBedWork()) {
            BedTnt tntentity = new BedTnt(worldIn, (double)pos.getX() + 0.5D, (double)pos.getY(), (double)pos.getZ() + 0.5D, explosionIn.getExplosivePlacedBy());
