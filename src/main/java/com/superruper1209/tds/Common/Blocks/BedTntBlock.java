...
public class BedTntBlock extends CustomTntBlock{
    ...
    public static final Block ENTRY = new BedTntBlock(AbstractBlock.Properties.of(Material.PLANT).instabreak().sound(SoundType.CROP)).setRegistryName("tds", "bed_tnt");
    ...
    public void wasExploded(World worldIn, BlockPos pos, Explosion explosionIn) {
        if (!worldIn.isClientSide && !worldIn.dimensionType().bedWorks()) {
            BedTnt tntentity = new BedTnt(worldIn, (double)pos.getX() + 0.5D, (double)pos.getY(), (double)pos.getZ() + 0.5D, explosionIn.getSourceMob());
